...
#define RREF      430.0 // cache reference value for max31855
#define RNOMINAL  100.0 /// cache nominal value for max31855
...
const int inPin = 42; // the input pin (for the toggle switch)
const int relaySW = 44; // the switch input for relay control
const int processLed = 46; // LED for relay indicator
const int ledPin = 48; // the pin for the log LED
...
Adafruit_MAX31856 thermocouple1(2, 3, 4, 5); // assign variable thermocouple1 as Adafruit_MAX31856 type
Adafruit_MAX31856 thermocouple2(6, 7, 8, 9); // assign variable thermocouple2 as Adafruit_MAX31856 type

// Use software SPI:  CLK, CS, DO
Adafruit_MAX31855 thermocouple3(11, 12, 13); // assign variable thermocouple3 as Adafruit_MAX31855 type
...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
// function that reads voltage from voltage divider 128 times and take average out of them then return the voltage value
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
float readVolt() {
...
void setup() {
  ...
  rtc.setTime(11, 40, 00);  // Set the time to, for eg.: 12:00:00 (24hr format)
  rtc.setDate(19, 10, 2018);  // Set the date

  pinMode(pinCS, OUTPUT); // declare Chip select pin of the SD card as output
  pinMode(ledPin, OUTPUT); // declare LED as output
  pinMode(processLed, OUTPUT); // declare  LED as output
  ...
  thermocouple1.begin(); //start thermocouple1
  thermocouple2.begin(); //start thermocouple2

  thermocouple1.setThermocoupleType(MAX31856_TCTYPE_K); // cast thermocouple1 to K-Type
  thermocouple2.setThermocoupleType(MAX31856_TCTYPE_K); // cast thermocouple2 to K-Type

  // SD Card Initialization
  if (SD.begin())
  {
    Serial.println();// if string is successful
...
void loop() {
  ...
  double c1 = thermocouple1.readThermocoupleTemperature(); // 'double' is used to define "c1", the outside temperature recorded by the thermocouple with double precision floating point number
  ...
  double c2 = thermocouple2.readThermocoupleTemperature();

  //Requesting temperatures from thermocouple 3
  double c3 = thermocouple3.readCelsius();

  avgT = (c1 + c2) / 2; // average between Thermocouple 1 and 2
  ...
  while (val == HIGH) { //when toggle switch is ON
    ...
    double tc1 = thermocouple1.readThermocoupleTemperature(); // assign value from sensor1 to variable tc1
    double tc2 = thermocouple2.readThermocoupleTemperature(); // assign value from sensor2 to variable tc2
    double tc3 = thermocouple3.readCelsius(); // assign value from sensor3 to variable tc3
    ...
    if (dataFile) {
      ...
      dataFile.print(diff);
      dataFile.print("\t");
      dataFile.print(readVolt());
      dataFile.print("\t");
      dataFile.print(readCurrent());
      dataFile.println(); // Print new line
      dataFile.close(); // close the file, physically save to sd card
    }
    val = digitalRead(inPin); // reads the status of the pushbutton
